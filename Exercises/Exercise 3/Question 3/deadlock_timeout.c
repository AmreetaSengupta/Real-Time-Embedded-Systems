...
void *grabRsrcs(void *threadid)
{
   ...
   if((long)threadid == THREAD_1) printf("Thread 1 started\n");
   else if((long)threadid == THREAD_2) printf("Thread 2 started\n");
   ...
   if((long)threadid == THREAD_1)
   {
     printf("THREAD 1 grabbing resource A @ %ld sec and %ld nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     ...
     printf("THREAD 1 got A, trying for B @ %ld sec and %ld nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     ...
     {
         clock_gettime(CLOCK_REALTIME, &timeNow);
         printf("Thread 1 GOT B @ %ld sec and %ld nsec with rc=%d\n", timeNow.tv_sec, timeNow.tv_nsec, rc);
         rsrcBCnt++;
         printf("rsrcACnt=%d, rsrcBCnt=%d\n", rsrcACnt, rsrcBCnt);
     }
   ...
   {
     printf("THREAD 2 grabbing resource B @ %ld sec and %ld nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     ...
     printf("THREAD 2 got B, trying for A @ %ld sec and %ld nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     rc=pthread_mutex_timedlock(&rsrcA, &rsrcA_timeout);
     //rc=pthread_mutex_lock(&rsrcA);
     if(rc == 0)
     {
         clock_gettime(CLOCK_REALTIME, &timeNow);
         printf("Thread 2 GOT A @ %ld sec and %ld nsec with rc=%d\n", timeNow.tv_sec, timeNow.tv_nsec, rc);
         rsrcACnt++;
         printf("rsrcACnt=%d, rsrcBCnt=%d\n", rsrcACnt, rsrcBCnt);
     }
     else if(rc == ETIMEDOUT)
